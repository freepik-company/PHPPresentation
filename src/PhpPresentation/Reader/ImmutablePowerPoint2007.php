 ...
 */
class ImmutablePowerPoint2007 implements ReaderInterface
{
    /**
     * Output Object.
     *
    ...
    protected function loadLayoutSlide(string $sPart, string $baseFile, SlideMaster $oSlideMaster): ?Slide\ImmutableSlideLayout
    {
        $xmlReader = new XMLReader();
        // @phpstan-ignore-next-line
        if ($layoutFile = $xmlReader->getDomFromString($sPart)) {
            // Core
            $oSlideLayout = new Slide\ImmutableSlideLayout($oSlideMaster);
            ...
            // Shapes
            /** @erika change */
            $this->loadDomXMLTree($oSlideLayout, $layoutFile);
    ...
    protected function loadTheme(string $sPart, SlideMaster $oSlideMaster): void
    {
        $xmlReader = new XMLReader();
        // @phpstan-ignore-next-line
        if ($themeFile = $xmlReader->getDomFromString($sPart)) {

            /** @erika change */
            $oSlideMaster->loadDomTheme($themeFile);
